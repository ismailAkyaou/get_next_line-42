...
/*   Updated: 2022/12/17 22:08:12 by iakyaou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buf)
		return (free(se_buf[fd]), se_buf[fd] = NULL, NULL);
	while (read_lin && (!ft_strchr(se_buf[fd], '\n')))
	{
		read_lin = read(fd, buf, BUFFER_SIZE);
		if (read_lin < 0)
			return (free(se_buf[fd]), se_buf[fd] = NULL, free (buf), NULL);
		buf[read_lin] = '\0';
		se_buf[fd] = ft_strjoin(se_buf[fd], buf);
	}
	ft_free(&se_buf[fd]);
	line = ft_line(se_buf[fd]);
	se_buf[fd] = ft_next_line(se_buf[fd]);
